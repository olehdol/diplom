//  TODO: 1) Upload FinanceData to database 
// 2) and get data  after user authorithation  (repase this data to FinanceData)
//  Feat:( added logout btn (check prev youtube video) )
//  Feat:( add posobility to remove exsisting items) )
//  Feat:( added modal notification (toastr)) )
//  Improments:( resolve response issues ) 
...
((UIController, ExpenseController) => {
    ...
    let FinanceData = []; // All finance data
    let addExpense = () => {
        ...
        if (input.description !== "" && !isNaN(input.value) && input.value > 0) {
            FinanceData.push(input)
            console.log('Adding item');
            FinanceData.map(item => {
                UIController.addListItem(item);
                ExpenseController.inputEntry(item);
